...
export default function GuestBillView() {
  ...
  const [currentSessionSesconds, setCurrentSessionSeconds] = useState(0);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (sessionStart && accessLogId && !accessDenied) {
        ...
        // Update the local session seconds
        setCurrentSessionSeconds(sessionSeconds);

        // Calculate total used including current session
        const totalUsedWithCurrent = totalSecondsUsed + sessionSeconds;

        // Update timeLeft based on total used
        const remainingSeconds = Math.max(0, 21600 - totalUsedWithCurrent);
        const hours = Math.floor(remainingSeconds / 3600);
        const minutes = Math.floor((remainingSeconds % 3600) / 60);
        const seconds = remainingSeconds % 60;
        setTimeLeft(`${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`);

        // Only update database every 30 seconds to reduce API calls
