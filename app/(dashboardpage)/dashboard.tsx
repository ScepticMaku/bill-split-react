...
export default function Dashboard() {
  ...
  const [loadBillLoading, setLoadBillLoading] = useState(false)
  ...
  const createBill = async () => {
    ...
    if (validator.equals(userRole, 'Standard') && selectedInvolvedPeople.length > 3) {
      showError(
        "Error adding more people",
        'Standard users can only add up to 3 people per bill.'
      )
      return;
    }
    ...
    const { data, error: createBillError } = await supabase
      .from('bills')
      .insert([{
        name: billName,
  ...
  const resetForm = () => {
    setBillName("");
    setSelectedInvolvedPeople(currentUser ? [currentUser] : []);
  ...
  const SelectPeopleModal = ({ visible, onClose, onConfirm, currentSelection, onAddGuestPress }) => {
    ...
    useEffect(() => {
      ...
      //   await supabase
      //     .from('bill_members')
      //     .update({ public_user_id: clerkUserId, guest_id: null })
      //     .eq('guest_id', 18);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentHeader}>
        ...
        <Pressable style={styles.addButton} onPress={() => { setShowAddModal(true); resetForm(); }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modernWireframeBox}>
            ...
            <Pressable style={styles.submitBtn} onPress={() => createBill()}>
              <ThemedText style={styles.submitBtnText}>{createBillLoading ? <ActivityIndicator color="white" /> : "Create Bill"}</ThemedText>
            </Pressable>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={showAddGuestModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modernWireframeBox}>
            ...
            <Pressable style={styles.submitBtn} onPress={handleSubmit}><ThemedText style={styles.submitBtnText}>{addGuestLoading ? <ActivityIndicator color="white" /> : "Submit button"}</ThemedText></Pressable>
          </View>
        </View>
      </Modal>
